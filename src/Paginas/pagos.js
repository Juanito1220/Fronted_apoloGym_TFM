...
const TAX_RATE = 0.12;       // IVA 12% (ajústalo a tu país)
const COUPONS = {            // cupones demo
  BIENVENIDA10: 0.10,        // 10% de descuento
  GYM5: 0.05
};
...
export default function Pagos() {
  ...
  const canPay = nameOk && emailOk && (method !== "card" || cardOk);
  ...
  function handlePay(e) {
    ...
    try {
      const prev = JSON.parse(localStorage.getItem("apolo_payments") || "[]");
      prev.push(receipt);
      localStorage.setItem("apolo_payments", JSON.stringify(prev));
    } catch {}
      ...
      <form className="grid" onSubmit={handlePay}>
        ...
        <aside className="card right">
          ...
          <button className="btn primary big" disabled={!canPay} type="submit">
            {method === "card" ? "Pagar ahora" : "Registrar pago"}
          </button>
